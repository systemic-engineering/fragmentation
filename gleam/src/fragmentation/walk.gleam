...
pub fn collect(root: Fragment(data)) -> List(Fragment(data)) {
  do_collect(root, [])
  |> list.reverse
}
...
pub fn fold(
  root: Fragment(data),
  acc: a,
  f: fn(a, Fragment(data)) -> Visitor(a),
) -> a {
  do_fold(root, acc, f)
}
...
pub fn depth(root: Fragment(data)) -> Int {
...
pub fn find(
  root: Fragment(data),
  predicate: fn(Fragment(data)) -> Bool,
) -> Result(Fragment(data), Nil) {
  case predicate(root) {
    True -> Ok(root)
    False ->
