...
pub fn encode_char_deterministic_test() {
  ...
  assert fragmentation.hash_string_fragment(a) == fragmentation.hash_string_fragment(b)
}
...
pub fn encode_word_is_fractal_test() {
  let result = encoding.encode_word("hi")
  assert fragmentation.is_fractal(result) == True
}
...
pub fn encode_paragraph_is_fractal_test() {
...
pub fn encode_paragraph_children_are_sentences_test() {
  ...
  assert list.all(children, fragmentation.is_fractal)
}
...
pub fn encode_sentence_is_fractal_test() {
...
pub fn ingest_returns_root_and_store_test() {
  ...
  assert fragmentation.is_fractal(root) == True
...
pub fn diff_same_document_unchanged_test() {
  ...
  let changes = diff.diff(doc, doc, fn(x: String) { x })
...
pub fn diff_modified_word_test() {
  ...
  let changes = diff.diff(doc_a, doc_b, fn(x: String) { x })
...
pub fn persist_encoding_to_disk_test() {
  ...
  let write_results =
    list.map(all, fn(f) { git.write(f, dir, fn(x: String) { x }) })
...
pub fn persist_deduped_encoding_test() {
  ...
  let all = walk.collect(root)
  list.each(all, fn(f) {
    let _ = git.write(f, dir, fn(x: String) { x })
  })
