...
fn draft_root_construction() {
    ...
    let draft: Draft<Fractal<String>, Sha> = Draft::root("initial", shard);
...
fn draft_with_parent() {
    ...
    let parent = Parent(sha::Sha("abc123".into()));
    let draft: Draft<Fractal<String>, Sha> = Draft::new("child commit", shard, parent.clone());
...
fn draft_node_accessor() {
    ...
    let draft: Draft<Fractal<String>, Sha> = Draft::root("test", shard);
...
fn draft_message_accessor() {
    ...
    let draft: Draft<Fractal<String>, Sha> = Draft::root("the message", shard);
...
fn draft_message_from_string() {
    ...
    let draft: Draft<Fractal<String>, Sha> = Draft::root(msg, make_shard("x"));
...
fn authored_stamps_name() {
    let draft: Draft<Fractal<String>, Sha> = Draft::root("test", make_shard("x"));
...
fn authored_preserves_message() {
    let draft: Draft<Fractal<String>, Sha> = Draft::root("preserved", make_shard("x"));
    let draft = draft.authored(Author::new("mara", "mara@systemic.engineer"));
...
fn authored_preserves_node() {
    let draft: Draft<Fractal<String>, Sha> = Draft::root("test", make_shard("payload"));
    let draft = draft.authored(Author::new("mara", "mara@systemic.engineer"));
    assert_eq!(draft.node().data(), "payload");
}
...
fn authored_preserves_parent() {
    ...
    let draft: Draft<Fractal<String>, Sha> = Draft::new("test", make_shard("x"), parent.clone());
    let draft = draft.authored(Author::new("mara", "mara@systemic.engineer"));
    assert_eq!(draft.parent(), Some(&parent));
}
...
fn draftable_message() {
    let draft: Draft<Fractal<String>, Sha> = Draft::root("the msg", make_shard("x"));
    let d: &dyn Draftable<Node = Fractal<String>, Hash = Sha> = &draft;
