...
-opaque set(CVO) :: {set, gleam@dict:dict(CVO, list(nil))}.
...
-spec contains(set(CVZ), CVZ) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CWB), CWB) -> set(CWB).
delete(Set, Member) ->
...
-spec fold(set(CWK), CWM, fun((CWM, CWK) -> CWM)) -> CWM.
...
-spec filter(set(CWN), fun((CWN) -> boolean())) -> set(CWN).
...
-spec drop(set(CWU), list(CWU)) -> set(CWU).
drop(Set, Disallowed) ->
...
-spec take(set(CWY), list(CWY)) -> set(CWY).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 289).
-spec order(set(CXG), set(CXG)) -> {set(CXG), set(CXG)}.
order(First, Second) ->
...
-spec intersection(set(CXL), set(CXL)) -> set(CXL).
...
-spec difference(set(CXP), set(CXP)) -> set(CXP).
...
-spec is_subset(set(CXT), set(CXT)) -> boolean().
...
-spec is_disjoint(set(CXW), set(CXW)) -> boolean().
...
-spec each(set(CYD), fun((CYD) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CVW), CVW) -> set(CVW).
insert(Set, Member) ->
...
-spec from_list(list(CWH)) -> set(CWH).
from_list(Members) ->
...
-spec map(set(CWQ), fun((CWQ) -> CWS)) -> set(CWS).
map(Set, Fun) ->
...
-spec union(set(CXC), set(CXC)) -> set(CXC).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(CXZ), set(CXZ)) -> set(CXZ).
