...
-type visitor(DTA) :: {continue, DTA} | {stop, DTA}.

-file("src/fragmentation/walk.gleam", 32).
...
-file("src/fragmentation/walk.gleam", 27).
...
-file("src/fragmentation/walk.gleam", 50).
-spec do_fold(
    fragmentation:fragment(),
    DTG,
    fun((DTG, fragmentation:fragment()) -> visitor(DTG))
) -> DTG.
do_fold(Frag, Acc, F) ->
    case F(Acc, Frag) of
        {stop, Result} ->
...
-file("src/fragmentation/walk.gleam", 42).
?DOC(" Fold over all fragments in a tree, depth-first.\n").
-spec fold(
    fragmentation:fragment(),
    DTE,
    fun((DTE, fragmentation:fragment()) -> visitor(DTE))
) -> DTE.
fold(Root, Acc, F) ->
    do_fold(Root, Acc, F).

-file("src/fragmentation/walk.gleam", 67).
...
-file("src/fragmentation/walk.gleam", 98).
-spec do_find(
...
-file("src/fragmentation/walk.gleam", 86).
