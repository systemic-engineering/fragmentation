...
-spec map_first({CMI, CMJ}, fun((CMI) -> CMK)) -> {CMK, CMJ}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({CML, CMM}, fun((CMM) -> CMN)) -> {CML, CMN}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
