...
-spec map({ok, CMZ} | {error, CNA}, fun((CMZ) -> CND)) -> {ok, CND} |
    {error, CNA}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, CNG} | {error, CNH}, fun((CNH) -> CNK)) -> {ok, CNG} |
    {error, CNK}.
...
-spec flatten({ok, {ok, CNN} | {error, CNO}} | {error, CNO}) -> {ok, CNN} |
    {error, CNO}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, CNV} | {error, CNW}, fun((CNV) -> {ok, CNZ} | {error, CNW})) -> {ok,
        CNZ} |
    {error, CNW}.
'try'(Result, Fun) ->
    case Result of
...
-spec unwrap({ok, COE} | {error, any()}, COE) -> COE.
...
-spec lazy_unwrap({ok, COI} | {error, any()}, fun(() -> COI)) -> COI.
...
-spec unwrap_error({ok, any()} | {error, CON}, CON) -> CON.
...
-spec 'or'({ok, COQ} | {error, COR}, {ok, COQ} | {error, COR}) -> {ok, COQ} |
    {error, COR}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, COY} | {error, COZ}, fun(() -> {ok, COY} | {error, COZ})) -> {ok,
        COY} |
    {error, COZ}.
...
-spec all(list({ok, CPG} | {error, CPH})) -> {ok, list(CPG)} | {error, CPH}.
all(Results) ->
    gleam@list:try_map(Results, fun(Result) -> Result end).

-file("src/gleam/result.gleam", 337).
-spec partition_loop(list({ok, CPV} | {error, CPW}), list(CPV), list(CPW)) -> {list(CPV),
    list(CPW)}.
...
-spec partition(list({ok, CPO} | {error, CPP})) -> {list(CPO), list(CPP)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CQE}, CQH) -> {ok, CQH} | {error, CQE}.
...
-spec replace_error({ok, CQK} | {error, any()}, CQO) -> {ok, CQK} | {error, CQO}.
...
-spec values(list({ok, CQR} | {error, any()})) -> list(CQR).
values(Results) ->
