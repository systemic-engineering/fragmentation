...
-spec be_ok({ok, DOP} | {error, any()}) -> DOP.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, DOU}) -> DOU.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(DOX)) -> DOX.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
